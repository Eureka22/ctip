...
void TNIBs::LoadGroundTruthTxt(TSIn& SIn) {
  ...
  while (!SIn.Eof()) {
	...
	Network.AddNode(Line.GetInt(), Line);
    if (!IsNodeNm(Line.GetInt())) {
      AddNodeNm(Line.GetInt(), TNodeInfo(Line, 0));
      DomainsIdH.AddDat(Line) = Line.GetInt();
    }
  }
...
void TNIBs::GenCascade(TCascade& C) {
  ...
  while (C.Len() < 2) {
    ...
    while (true) {
      ...
      C.Add(NId, GlobalTime, InfectedBy.GetDat(NId).Val);

      if (verbose) { printf("GlobalTime:%f, infected node:%d by %d \n", GlobalTime, NId,InfectedBy.GetDat(NId).Val); }
	  //if (InfectedBy.IsKey(NId))
...
void TNIBs::SaveInferredNetwork(const TStr& OutFNm) {
  TFOut FOut(OutFNm);
  /*
  // write nodes to file
  for (THash<TInt, TNodeInfo>::TIter NI = NodeNmH.BegI(); NI < NodeNmH.EndI(); NI++) {
...
void TNIBs::SaveCascades(const TStr& OutFNm) {
  ...
  for (THash<TInt, TCascade>::TIter CI = CascH.BegI(); CI < CascH.EndI(); CI++) {
    ...
    for (THash<TInt, THitInfo>::TIter NI = C.NIdHitH.BegI(); NI < C.NIdHitH.EndI(); NI++) {
      ...
      if (j > 0) { FOut.PutStr(TStr::Fmt(",%d,%d,%f", NI.GetDat().Parent.Val, NI.GetDat().NId.Val, NI.GetDat().Tm.Val)); }
...
{
	int Seed = 0;
    TKronMtx SeedMtx = TKronMtx::GetMtx(NetworkParams);
    printf("\n*** Seed matrix:\n");
    SeedMtx.Dump();
    printf("\n*** Kronecker:\n");
	int iter = int(log2(NNodes));
	
    // slow but exact O(n^2) algorightm
    //PNGraph Graph = TKronMtx::GenKronecker(SeedMtx, NIter, true, Seed); 
    // fast O(e) approximate algorithm
    PNGraph Graph = TKronMtx::GenFastKronecker(SeedMtx, iter , NEdges, true ,0); 
	printf("groundtruth nodes in Graph:%d edges:%d\n", Graph->GetNodes(), Graph->GetEdges());
	for (TNGraph::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++)
	{
		//printf("GetID%d\n",NI.GetId());
		Network.AddNode(NI.GetId(),TStr(NI.GetId()));
		AddNodeNm(NI.GetId(),  TNodeInfo(TStr(NI.GetId()),0));
	}
	
	for (TNGraph::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++)
	{
		TFltFltH Alphas;
		//Alphas.AddDat(0.0) = 0.5;// TFlt::Rnd.GetUniDevInt(10000)/10000.0;
		Alphas.AddDat(0.0) = TFlt::GetMx(TFlt::GetMn( TFlt::Rnd.GetExpDev(5), 1),0.01);
...
void TNIBs::UpdateForCascade2(const TOptMethod& OptMethod, TCascade& Cascade, const double& CurrentTime)
{
	...
	for (auto vi:vec)
	{
		if(!TempNetwork.IsNode(vi.second)) 
			TempNetwork.AddNode(vi.second,"G");
		TFltFltH Alphas;
		Alphas.AddDat(0.0) = CurrentTime+1;
		TempNetwork.AddEdge(vi.first,vi.second, Alphas);
	}
